...
// CODE HERE
const evenNumbers = mixedNumbers.filter(function(currentNum){
...
const postTaxPrices = prices.map(function(currentPrice){
  let tax = currentPrice * .07
  let totalPrice = currentPrice + tax
  return totalPrice
...
// CODE HERE //how exactly does totalSum and currentPopulation work
const totalPopulation = populations.reduce(function(totalSum, currentPopulation){
  return totalSum + currentPopulation
})
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(function(currentMonster) {
  return currentMonster.CP >= 200
})
...
//create a .map function with a new variable set to currentTotal
...
let bobSum = 0
var bobTotal = purchases.filter(function(obj){
